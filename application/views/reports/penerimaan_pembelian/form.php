...
<!-- form start -->
<form id="rptLPBGudang" action="<?= site_url() ?>report/tr/penerimaan_pembelian/process" method="POST" enctype="multipart/form-data">
    <div class="box-body">
        <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">                    
            ...
            <div class="form-group row">						
                ...
                <label for="select-type" class="col-sm-2 control-label"><?=lang("Jenis")?> :</label>
                <div class="col-sm-4">
                    <select id="select-type" class="form-control non-editable" name="fst_lpb_type">
                        <option value='PO'><?=lang("PO")?></option>
                        <option value='ASSEMBLING_IN'><?=lang("ASSEMBLING_IN")?></option>
                    </select>
                    <div id="fst_lpb_type_err" class="text-danger"></div>
                </div>   
            </div>
            <div class="form-group row">
                <label for="fdt_lpbgudang_datetime" class="col-sm-2 control-label"><?=lang("Tanggal")?> *</label>
                <div class="col-sm-4">
                    <div class="input-group date">
                        <div class="input-group-addon">
                            <i class="fa fa-calendar"></i>
                        </div>
                        <input type="text" class="form-control datepicker" id="fdt_lpbgudang_datetime" name="fdt_lpbgudang_datetime"/>
                    </div>
                    <div id="fdt_lpbgudang_datetime_err" class="text-danger"></div>
                    <!-- /.input group -->
                </div>
                <label for="fdt_lpbgudang_datetime2" class="col-sm-2 control-label"><?=lang("s/d")?> *</label>
                <div class="col-sm-4">
                    <div class="input-group date">
                        <div class="input-group-addon">
                            <i class="fa fa-calendar"></i>
                        </div>
                        <input type="text" class="form-control datepicker" id="fdt_lpbgudang_datetime2" name="fdt_lpbgudang_datetime2"/>
                    </div>
                    <div id="fdt_lpbgudang_datetime2_err" class="text-danger"></div>
                </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">								
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="1" checked onclick="handleRadioClick(this);"><?=lang("Laporan Penerimaan Barang Per-Supplier")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout2" class="rpt_layout" name="rpt_layout" value="2" onclick="handleRadioClick(this);"><?=lang("Laporan Penerimaan Barang Per-PO/Assembling")?></label>
                </div>
...
<script type="text/javascript" info="init">
	$(function(){
		$("#fdt_lpbgudang_datetime").val(dateFormat("<?= date("Y-m-d")?>")).datepicker("update");
        $("#fdt_lpbgudang_datetime2").val(dateFormat("<?= date("Y-m-d")?>")).datepicker("update");					
	});
</script>
<script type="text/javascript">
    ...
    $(function() {
        $("#select-relations").select2({
			width: '100%',
			ajax:{
				url: '<?=site_url()?>report/gudang/penerimaan_pembelian/get_suppliers',
				dataType: 'json',
				delay: 250,
        ...
        $("#select-items").select2({
            ...
            ajax:{
                delay: 250,
                url: "<?=site_url()?>/report/gudang/penerimaan_pembelian/ajxListItem",
                dataType: 'json',
        ...
        $("#btnProcess").click(function(event) {
            ...
            data = $("#rptLPBGudang").serializeArray();
            url = "<?= site_url() ?>report/gudang/penerimaan_pembelian/process";
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    if (resp.status == "VALIDATION_FORM_FAILED") {
                        ...
                        url = "<?= site_url() ?>report/gudang/penerimaan_pembelian/generatereport";
                        //alert(url);
                        //$("iframe").attr("src",url);
                        $("#rptLPBGudang").attr('action', url);
                        $("#rptLPBGudang").attr('target', 'rpt_iframe');
                        $("#rptLPBGudang").submit();
        ...
        $("#btnExcel").click(function(event) {
			...
			a.download = 'Laporan_Penerimaan_Barang' + '.xls';
			a.click();                        			
			return;
		});      
    });
