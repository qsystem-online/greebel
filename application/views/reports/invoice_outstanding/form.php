...
<form id="irptInvoiceOutstanding" action="<?= site_url() ?>report/tr/invoice_oustanding/process" method="POST" enctype="multipart/form-data">
    <div class="box-body">
        <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">                    
            ...
            <div id="area_1" class="form-group row">
            ...
            <div id="area_2" class="form-group row">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">								
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="1" checked onclick="handleRadioClick(this);"><?=lang("Laporan Penjualan Outstanding")?></label>
...
<script type="text/javascript">
    ...
    $(function() {
        $("#select-relations").select2({
			width: '100%',
			ajax:{
				url: '<?=site_url()?>report/tr/invoice_outstanding/get_customers',
				dataType: 'json',
				delay: 250,
        ...
        $("#select-provinces").select2({
            width: '100%',
            ajax: {
                url: '<?=site_url()?>report/tr/invoice_outstanding/get_provinces',
                dataType: 'json',
                delay: 250,
                processResults: function (data){
                    items = [];
        ...
        $("#select-provinces").change(function(event){
			...
			$("#select-district").select2({
				width: '100%',
				ajax: {
					url: '<?=site_url()?>report/tr/invoice_outstanding/get_district/'+$("#select-provinces").val(),
					dataType: 'json',
					delay: 250,
					processResults: function (data){
						items = [];
        ...
        $("#select-district").change(function(event){
			...
			$("#select-subdistrict").select2({
				width: '100%',
				ajax: {
					url: '<?=site_url()?>report/tr/invoice_outstanding/get_subdistrict/'+$("#select-district").val(),
					dataType: 'json',
					delay: 250,
					processResults: function (data){
						items = [];
		...
		$("#select-subdistrict").change(function(event){
			...
			$("#select-village").select2({
				width: '100%',
				ajax: {
					url: '<?=site_url()?>report/tr/invoice_outstanding/get_village/'+$("#select-subdistrict").val(),
					dataType: 'json',
					delay: 250,
					processResults: function (data){
						items = [];
        ...
        $("#btnExcel").click(function(event) {
			...
			a.download = 'Laporan_Faktur_Penjualan_Outstanding_Pembayaran' + '.xls';
			a.click();                        			
			return;
		});      
    });
