...
<!-- form start -->
<form id="rptCostPurchase" action="<?= site_url() ?>report/tr/Cost_purchase/process" method="POST" enctype="multipart/form-data">
    <div class="box-body">
        <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">
            ...
            <div class="form-group row">
                <label for="fdt_purchasecost_datetime" class="col-sm-2 control-label"><?=lang("Tanggal Biaya")?></label>
                <div class="col-sm-4">
                    <div class="input-group date">
                        <div class="input-group-addon">
                            <i class="fa fa-calendar"></i>
                        </div>
                        <input type="text" class="form-control datepicker" id="fdt_purchasecost_datetime" name="fdt_purchasecost_datetime"/>
                    </div>
                    <div id="fdt_purchasecost_datetime_err" class="text-danger"></div>
                    <!-- /.input group -->
                </div>
                <label for="fdt_purchasecost_datetime2" class="col-sm-2 control-label"><?=lang("s/d")?></label>
                <div class="col-sm-4">
                    <div class="input-group date">
                        <div class="input-group-addon">
                            <i class="fa fa-calendar"></i>
                        </div>
                        <input type="text" class="form-control datepicker" id="fdt_purchasecost_datetime2" name="fdt_purchasecost_datetime2"/>
                    </div>
                    <div id="fdt_purchasecost_datetime2_err" class="text-danger"></div>
                </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">								
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="1" checked onclick="handleRadioClick(this);"><?=lang("Laporan Biaya Pembelian Detail")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout2" class="rpt_layout" name="rpt_layout" value="2" onclick="handleRadioClick(this);"><?=lang("Laporan Biaya Pembelian Ringkas Per-PO")?></label>
                </div>
...
<script type="text/javascript" info="init">
	$(function(){
		$("#fdt_purchasecost_datetime").val(dateFormat("<?= date("Y-m-d")?>")).datepicker("update");
        $("#fdt_purchasecost_datetime2").val(dateFormat("<?= date("Y-m-d")?>")).datepicker("update");					
	});
</script>
<script type="text/javascript">
    ...
    $(function() {
        $("#select-relations").select2({
			width: '100%',
			ajax:{
				url: '<?=site_url()?>report/tr/cost_purchase/get_suppliers',
				dataType: 'json',
				delay: 250,
        ...
        $("#btnProcess").click(function(event) {
            ...
            data = $("#rptCostPurchase").serializeArray();
            url = "<?= site_url() ?>report/tr/cost_purchase/process";
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    if (resp.status == "VALIDATION_FORM_FAILED") {
                        ...
                        url = "<?= site_url() ?>report/tr/cost_purchase/generatereport";
                        //alert(url);
                        //$("iframe").attr("src",url);
                        $("#rptCostPurchase").attr('action', url);
                        $("#rptCostPurchase").attr('target', 'rpt_iframe');
                        $("#rptCostPurchase").submit();
        ...
        $("#btnExcel").click(function(event) {
			...
			a.download = 'Laporan_biaya_pembelian' + '.xls';
			a.click();                        			
			return;
		});      
    });
