...
<!-- form start -->
<form id="rptGLCBReceive" action="<?= site_url() ?>report/kas_bank/penerimaan_piutang/process" method="POST" enctype="multipart/form-data">
    <div class="box-body">
        <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">                    
            ...
            <div class="form-group row">
                <label for="fdt_cbreceive_datetime" class="col-sm-2 control-label"><?=lang("Tanggal")?> *</label>
                <div class="col-sm-4">
                    <div class="input-group date">
                        <div class="input-group-addon">
                            <i class="fa fa-calendar"></i>
                        </div>
                        <input type="text" class="form-control datepicker" id="fdt_cbreceive_datetime" name="fdt_cbreceive_datetime"/>
                    </div>
                    <div id="fdt_cbreceive_datetime_err" class="text-danger"></div>
                    <!-- /.input group -->
                </div>
                <label for="fdt_cbreceive_datetime2" class="col-sm-2 control-label"><?=lang("s/d")?> *</label>
                <div class="col-sm-4">
                    <div class="input-group date">
                        <div class="input-group-addon">
                            <i class="fa fa-calendar"></i>
                        </div>
                        <input type="text" class="form-control datepicker" id="fdt_cbreceive_datetime2" name="fdt_cbreceive_datetime2"/>
                    </div>
                    <div id="fdt_cbreceive_datetime2_err" class="text-danger"></div>
                </div>
            </div>
            <div class="form-group row">
                <label for="rpt_layout" class="col-sm-2 control-label"><?=lang("Report Layout")?></label>
                <div class="col-sm-4">								
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="1" checked onclick="handleRadioClick(this);"><?=lang("Laporan Penggunaan GLAccount (Penerimaan piutang) ")?></label>
                </div>
...
<script type="text/javascript" info="init">
	$(function(){
		$("#fdt_cbreceive_datetime").val(dateFormat("<?= date("Y-m-d")?>")).datepicker("update");
        $("#fdt_cbreceive_datetime2").val(dateFormat("<?= date("Y-m-d")?>")).datepicker("update");					
	});
</script>
<script type="text/javascript">
    ...
    $(function() {
        $("#btnProcess").click(function(event) {
            ...
            data = $("#rptGLCBReceive").serializeArray();
            url = "<?= site_url() ?>report/kas_bank/penerimaan_piutang/process";
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    if (resp.status == "VALIDATION_FORM_FAILED") {
                        ...
                        url = "<?= site_url() ?>report/kas_bank/penerimaan_piutang/generatereport";
                        //alert(url);
                        //$("iframe").attr("src",url);
                        $("#rptGLCBReceive").attr('action', url);
                        $("#rptGLCBReceive").attr('target', 'rpt_iframe');
                        $("#rptGLCBReceive").submit();
        ...
        $("#btnExcel").click(function(event) {
			...
			a.download = 'Laporan_GLAccount_penerimaan_piutang' + '.xls';
			a.click();                        			
			return;
		});      
    });
