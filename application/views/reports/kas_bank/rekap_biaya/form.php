...
<!-- form start -->
<form id="rptRekapbiaya" action="<?= site_url() ?>report/kas_bank/rekap_biaya/process" method="POST" enctype="multipart/form-data">
    <div class="box-body">
        <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">                    
            <div class="form-group row">						
                ...
                <label for="select-pcc" class="col-sm-2 control-label"><?=lang("Profit/Cost Center")?></label>
                <div class="col-sm-4">
                    <select id="select-pcc" class="form-control non-editable" name="fin_pcc_id">
                    <option value='0'>All</option>
                    <?php
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">								
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="1" checked onclick="handleRadioClick(this);"><?=lang("Laporan Rekap Biaya")?></label>
                </div>
...
<script type="text/javascript">
    ...
    $(function() {
        $("#btnProcess").click(function(event) {
            ...
            data = $("#rptRekapbiaya").serializeArray();
            url = "<?= site_url() ?>report/kas_bank/rekap_biaya/process";
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    if (resp.status == "VALIDATION_FORM_FAILED") {
                        ...
                        url = "<?= site_url() ?>report/kas_bank/rekap_biaya/generatereport";
                        //alert(url);
                        //$("iframe").attr("src",url);
                        $("#rptRekapbiaya").attr('action', url);
                        $("#rptRekapbiaya").attr('target', 'rpt_iframe');
                        $("#rptRekapbiaya").submit();
        ...
        $("#btnExcel").click(function(event) {
			...
			a.download = 'Laporan_Rekap_Biaya' + '.xls';
			a.click();                        			
			return;
		});      
    });
