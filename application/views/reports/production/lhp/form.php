...
<!-- form start -->
<form id="rptLHP" action="<?= site_url() ?>report/production/lhp/process" method="POST" enctype="multipart/form-data">
    <div class="box-body">
        <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>"> 
            <div class="form-group row">
                <label for="fdt_datetime" class="col-sm-2 control-label" id="tanggal" name="tanggal"><?=lang("Tanggal WO")?> *</label>
                <div class="col-sm-4">
            ...
            <div class="form-group row">
                ...
                <label for="select-wo2" class="col-sm-2 control-label"><?=lang("- Work Order")?></label>
                <div class="col-sm-4">
                    <select id="fin_wo_id2" class="form-control" name="fin_wo_id2">
                    </select>
                    <div id="fin_wo_id2_err" class="text-danger"></div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">								
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="1" checked onclick="handleRadioClick(this);"><?=lang("Laporan WO dan Hasil produksi")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="2" onclick="handleRadioClick(this);"><?=lang("Laporan Hasil produksi (Tgl Data)")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="3" onclick="handleRadioClick(this);"><?=lang("Laporan Hasil produksi (Tgl System)")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="4" onclick="handleRadioClick(this);"><?=lang("Laporan Hasil produksi (Tgl Edit)")?></label>
                </div>
...
<script type="text/javascript" info="init">
	$(function(){		
		$("#fin_wo_id").select2({
			ajax:{
				url:"<?=site_url()?>report/production/lhp/ajxGetWOList",
...
<script type="text/javascript">
    ...
    function handleRadioClick(myRadio) {
        ...
        if (myRadio.value == "1"){
            $('#tanggal').text("Tanggal WO*");
        }
        if (myRadio.value == "2"){
            $('#tanggal').text("Tanggal data LHP*");
        }

        if (myRadio.value == "3"){
            $('#tanggal').text("Tanggal system LHP*");
        }

        if (myRadio.value == "4"){
            $('#tanggal').text("Tanggal edit LHP*");
        }
    ...
    $(function() {
        ...
        $("#btnProcess").click(function(event) {
            ...
            data = $("#rptLHP").serializeArray();
            url = "<?= site_url() ?>report/production/lhp/process";
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    if (resp.status == "VALIDATION_FORM_FAILED") {
                        ...
                        url = "<?= site_url() ?>report/production/lhp/generatereport";
                        //alert(url);
                        //$("iframe").attr("src",url);
                        $("#rptLHP").attr('action', url);
                        $("#rptLHP").attr('target', 'rpt_iframe');
                        $("#rptLHP").submit();
        ...
        $("#btnExcel").click(function(event) {
			...
			a.download = 'Laporan_Hasil_Produksi' + '.xls';
			a.click();                        			
			return;
		});      
    });
