...
<!-- form start -->
<form id="rptRMOUTReturn" action="<?= site_url() ?>report/production/rmoutreturn/process" method="POST" enctype="multipart/form-data">
    <div class="box-body">
        <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>"> 
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">								
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="1" checked onclick="handleRadioClick(this);"><?=lang("Laporan RMOUT-RETURN (Tgl Data)")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="2" onclick="handleRadioClick(this);"><?=lang("Laporan RMOUT-RETURN (Tgl System)")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="3" onclick="handleRadioClick(this);"><?=lang("Laporan RMOUT-RETURN (Tgl Edit)")?></label>
                </div>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        $("#btnProcess").click(function(event) {
            ...
            data = $("#rptRMOUTReturn").serializeArray();
            url = "<?= site_url() ?>report/production/rmoutreturn/process";
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    if (resp.status == "VALIDATION_FORM_FAILED") {
                        ...
                        url = "<?= site_url() ?>report/production/rmoutreturn/generatereport";
                        //alert(url);
                        //$("iframe").attr("src",url);
                        $("#rptRMOUTReturn").attr('action', url);
                        $("#rptRMOUTReturn").attr('target', 'rpt_iframe');
                        $("#rptRMOUTReturn").submit();
        ...
        $("#btnExcel").click(function(event) {
			...
			a.download = 'Laporan_RMOUT_RETURN' + '.xls';
			a.click();                        			
			return;
		});      
    });
