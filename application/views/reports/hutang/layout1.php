<html>
	...
	<body id="bodyReport">
		...
		<div>LAPORAN KARTU HUTANG RINCI</div>
		<br>
        <?php
				...
				<?php
					...
					foreach ($dataReport as $row){
						if ( $relation_id != $row->fin_relation_id ){
                            ...
                            $ssql = "SELECT IFNULL(SUM(fdc_debit),0) AS fdc_debit,IFNULL(SUM(fdc_credit),0) AS fdc_credit FROM glledger WHERE fin_relation_id = '".$relation_id."' AND 
                            ...
                            AND fdt_trx_datetime < '". date('Y-m-d', strtotime($start_date)). "' AND fst_active = 'A' ";
                            $qr = $this->db->query($ssql);
                            //echo $this->db->last_query();
                            //die();
                            $rw = $qr->row();
                            
                            $saldo_awal = ($rw->fdc_credit - $rw->fdc_debit);
						...
						$nou++;
                        if($nou == 2){
                            $saldo_akhir = ($saldo_awal + $row->fdc_credit) - $row->fdc_debit;
                        }else{
                            $saldo_akhir = ($saldo_akhir + $row->fdc_credit) - $row->fdc_debit;
                        }
