<html>
	...
	<body id="bodyReport">
		...
		<div>LAPORAN PENERIMAAN BARANG RETURN PER-CUSTOMER</div>
		<br>
		<?php
		...
		<table id="tblReport" cellpadding="0" cellspacing="0" style="width:1600px">      
			<thead>
				<tr style="background-color:RoyalBlue;color:white">
					<?php
						...
						echoIfColSelected(4,$selectedCols,"<th class='col-4' style='width:100px'>Kode Barang</th>");
						...
						echoIfColSelected(8,$selectedCols,"<th class='col-8' style='width:100px'>No.Invoice</th>");
						echoIfColSelected(9,$selectedCols,"<th class='col-9' style='width:100px'>Tgl.Invoice</th>");
					?>
				</tr>
			</thead>
			<tbody>
				<?php
					...
					foreach ($dataReport as $row){
						...
						if ( $Id_LpbReturn != $row->Id_LpbReturn ){
                            ...
                            if ( $idCustomer != $row->fin_customer_id ){
                                $idCustomer = $row->fin_customer_id;
								//echoIfColSelected(0,$selectedCols,"<td class='col-0'>$row->Relation_Name</td>");
								echo "<tr>";
								echo "<td colspan='".totalSelectedCol(12,$selectedCols)."'style='text-align: left;font-weight: bold'>$row->Customer_Name</td>";
								echo "</tr>";	   
							}
                        ...
                        echoIfColSelected(4,$selectedCols,"<td class='col-4'>$row->Item_Code</td>");
                        echoIfColSelected(5,$selectedCols,"<td class='col-5'>$row->Item_Name</td>");
                        echoIfColSelected(6,$selectedCols,"<td class='col-6'style='text-align: right'>$Qty</td>");
                        echoIfColSelected(7,$selectedCols,"<td class='col-7'>$row->Unit</td>");
                        echoIfColSelected(8,$selectedCols,"<td class='col-8'>$row->No_Inv</td>");
						echoIfColSelected(9,$selectedCols,"<td class='col-9'>$row->Inv_Date</td>");											                                                                                                                                                                      
                        echo "</tr>";
					}
				?>
			</tbody>
