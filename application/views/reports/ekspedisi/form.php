...
<!-- form start -->
<form id="rptEkspedisi" action="<?= site_url() ?>report/tr/ekspedisi/process" method="POST" enctype="multipart/form-data">
    <div class="box-body">
        <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">                    
            ...
            <div class="form-group row">						
                <label for="select-customer" class="col-sm-2 control-label"><?=lang("Customer")?></label>
                <div class="col-sm-4">
                    <select id="select-customer" class="form-control non-editable" name="fin_customer_id">
                    </select>
                    <div id="fin_customer_id_err" class="text-danger"></div>
                </div>
                <label for="select-ekspedisi" class="col-sm-2 control-label"><?=lang("Ekspedisi")?></label>
                <div class="col-sm-4">
                    <select id="select-ekspedisi" class="form-control non-editable" name="fin_ekspedisi_id">
                    </select>
                    <div id="fin_ekspedisi_id_err" class="text-danger"></div>
                </div>         
            </div>
            <div class="form-group row">
                <label for="fdt_salesekspedisi_datetime" class="col-sm-2 control-label"><?=lang("Tgl.Ekspedisi")?> *</label>
                <div class="col-sm-4">
                    <div class="input-group date">
                        <div class="input-group-addon">
                            <i class="fa fa-calendar"></i>
                        </div>
                        <input type="text" class="form-control datepicker" id="fdt_salesekspedisi_datetime" name="fdt_salesekspedisi_datetime"/>
                    </div>
                    <div id="fdt_salesekspedisi_datetime_err" class="text-danger"></div>
                    <!-- /.input group -->
                </div>
                <label for="fdt_salesekspedisi_datetime2" class="col-sm-2 control-label"><?=lang("s/d")?> *</label>
                <div class="col-sm-4">
                    <div class="input-group date">
                        <div class="input-group-addon">
                            <i class="fa fa-calendar"></i>
                        </div>
                        <input type="text" class="form-control datepicker" id="fdt_salesekspedisi_datetime2" name="fdt_salesekspedisi_datetime2"/>
                    </div>
                    <div id="fdt_salesekspedisi_datetime2_err" class="text-danger"></div>
                </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">								
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="1" checked onclick="handleRadioClick(this);"><?=lang("Laporan Ekspedisi Ringkas")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout2" class="rpt_layout" name="rpt_layout" value="2" onclick="handleRadioClick(this);"><?=lang("Laporan Ekspedisi Detail S/J")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout3" class="rpt_layout" name="rpt_layout" value="3" onclick="handleRadioClick(this);"><?=lang("Laporan Ekspedisi dan Status pembayaran ke Ekspedisi")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout4" class="rpt_layout" name="rpt_layout" value="4" onclick="handleRadioClick(this);"><?=lang("Laporan Ekspedisi dan Status pelunasan Customer")?></label>
                </div>
...
<script type="text/javascript" info="init">
	$(function(){
		$("#fdt_salesekspedisi_datetime").val(dateFormat("<?= date("Y-m-d")?>")).datepicker("update");
        $("#fdt_salesekspedisi_datetime2").val(dateFormat("<?= date("Y-m-d")?>")).datepicker("update");				
	});
</script>
<script type="text/javascript">
    ...
    $(function() {
        $("#select-customer").select2({
			width: '100%',
			ajax:{
				url: '<?=site_url()?>report/tr/ekspedisi/get_customers',
				dataType: 'json',
				delay: 250,
        ...
        $("#select-ekspedisi").select2({
			width: '100%',
			ajax:{
				url: '<?=site_url()?>report/tr/ekspedisi/get_ekspedisi',
				dataType: 'json',
				delay: 250,
				processResults: function (data){
					items = [];
        ...
        $("#btnProcess").click(function(event) {
            ...
            data = $("#rptEkspedisi").serializeArray();
            url = "<?= site_url() ?>report/tr/ekspedisi/process";
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    if (resp.status == "VALIDATION_FORM_FAILED") {
                        ...
                        url = "<?= site_url() ?>report/tr/ekspedisi/generatereport";
                        //alert(url);
                        //$("iframe").attr("src",url);
                        $("#rptEkspedisi").attr('action', url);
                        $("#rptEkspedisi").attr('target', 'rpt_iframe');
                        $("#rptEkspedisi").submit();
        ...
        $("#btnExcel").click(function(event) {
			...
			a.download = 'Laporan_Transaksi_Ekspedisi' + '.xls';
			a.click();                        			
			return;
		});      
    });
