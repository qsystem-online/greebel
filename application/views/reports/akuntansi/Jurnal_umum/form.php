...
<!-- form start -->
<form id="rptJurnalUmum" action="<?= site_url() ?>report/akuntansi/jurnal_umum/process" method="POST" enctype="multipart/form-data">
    <div class="box-body">
        <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">                    
            ...
            <div class="form-group row">						
                <label for="select-jurnal" class="col-sm-2 control-label"><?=lang("Jenis Jurnal")?></label>
                <div class="col-sm-4">
                ...
                <label for="select-jurnal2" class="col-sm-2 control-label"><?=lang("Jenis Jurnal")?></label>
                <div class="col-sm-4">
                    <select class="form-control" id="fst_journal_type2"  name="fst_journal_type2">
                        <option value='JA'>Jurnal Penyesuaian</option>
                        <option value='JB'>Jurnal Pembelian</option>
                        <option value='JC'>Jurnal Penutup</option>
                        <option value='JJ'>Jurnal Penjualan</option>
                    ...
                    <div id="fst_journal_type2_err" class="text-danger"></div>
                </div>    
            </div>
            <div class="form-group row">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">								
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="1" checked onclick="handleRadioClick(this);"><?=lang("Laporan Jurnal Umum")?></label>
                </div>
...
<script type="text/javascript">
    ...
    $(function() {
        $("#btnProcess").click(function(event) {
            ...
            data = $("#rptJurnalUmum").serializeArray();
            url = "<?= site_url() ?>report/akuntansi/jurnal_umum/process";
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    if (resp.status == "VALIDATION_FORM_FAILED") {
                        ...
                        url = "<?= site_url() ?>report/akuntansi/jurnal_umum/generatereport";
                        //alert(url);
                        //$("iframe").attr("src",url);
                        $("#rptJurnalUmum").attr('action', url);
                        $("#rptJurnalUmum").attr('target', 'rpt_iframe');
                        $("#rptJurnalUmum").submit();
        ...
        $("#btnExcel").click(function(event) {
			...
			a.download = 'Laporan_Jurnal_Umum' + '.xls';
			a.click();                        			
			return;
		});      
    });
