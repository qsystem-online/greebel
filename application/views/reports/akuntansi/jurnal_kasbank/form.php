...
<!-- form start -->
<form id="rptJurnalHarian" action="<?= site_url() ?>report/akuntansi/jurnal_kasbank/process" method="POST" enctype="multipart/form-data">
    <div class="box-body">
        <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">                    
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">								
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="1" checked onclick="handleRadioClick(this);"><?=lang("Journal Penerimaan Kas/Bank")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout2" class="rpt_layout" name="rpt_layout" value="2" onclick="handleRadioClick(this);"><?=lang("Journal Pengeluaran Kas/Bank")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout3" class="rpt_layout" name="rpt_layout" value="3" onclick="handleRadioClick(this);"><?=lang("Journal Penerimaan Kas/Bank Lain2")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout4" class="rpt_layout" name="rpt_layout" value="4" onclick="handleRadioClick(this);"><?=lang("Journal Pengeluaran Kas/Bank Lain2")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout5" class="rpt_layout" name="rpt_layout" value="5" onclick="handleRadioClick(this);"><?=lang("Semua Journal Kas/Bank")?></label>
                    <label class="radio"><input type="radio" id="rpt_layout6" class="rpt_layout" name="rpt_layout" value="6" onclick="handleRadioClick(this);"><?=lang("Semua Journal Lainnya")?></label>
                </div>
...
<script type="text/javascript">
    ...
    $(function() {
        $("#btnProcess").click(function(event) {
            ...
            data = $("#rptJurnalHarian").serializeArray();
            url = "<?= site_url() ?>report/akuntansi/jurnal_kasbank/process";
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    if (resp.status == "VALIDATION_FORM_FAILED") {
                        ...
                        url = "<?= site_url() ?>report/akuntansi/jurnal_kasbank/generatereport";
                        //alert(url);
                        //$("iframe").attr("src",url);
                        $("#rptJurnalHarian").attr('action', url);
                        $("#rptJurnalHarian").attr('target', 'rpt_iframe');
                        $("#rptJurnalHarian").submit();
        ...
        $("#btnExcel").click(function(event) {
			...
			a.download = 'Laporan_Journal_Harian' + '.xls';
			a.click();                        			
			return;
		});      
    });
