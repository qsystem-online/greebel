...
<section class="content-header">
	<h1><?=lang("Kas & Bank Pengeluaran Lain-lain")?><small><?=lang("form")?></small></h1>
	<ol class="breadcrumb">
            ...
            <div class="box box-info">
				...
				<form id="frmHeader" class="form-horizontal"  method="POST">			
					<div class="box-body">
						...
						<input type="hidden" class="form-control" id="fin_cbpaymentoth_id" placeholder="<?=lang("(Autonumber)")?>" name="fin_cbpaymentoth_id" value="<?=$fin_cbpaymentoth_id?>" readonly>						
						
						<div class="form-group">
                            <div class="col-md-5 col-md-offset-2">
                                <div class="radio">
                                    <label><input type="radio" id="typeKas" class="type-pembayaran" name="fst_cbpaymentoth_type" value="Cash" checked><?= lang("Kas") ?></label>
                                    <label style="margin-left:20px"><input type="radio" id="typeBank" class="type-pembayaran" value="Bank" name="fst_cbpaymentoth_type"><?= lang("Bank") ?></label>
                                </div>
                            </div>                                
						</div>

						<div class="form-group">
                            <label for="fin_kasbank_id" class="col-md-2 control-label"><?=lang("Jenis Pembayaran")?> #</label>
                            <div class="col-md-5">
                                <select id="fin_kasbank_id" name="fin_kasbank_id" class="form-control"></select>                                    
                            </div>
							<div id="fin_kasbank_id_err" class="text-danger"></div>

							<label for="fdt_cbreceive_datetime" class="col-md-2 control-label text-right"><?=lang("Tanggal Pembayaran")?> *</label>
							<div class="col-md-3">
								<div class="input-group date">
									<div class="input-group-addon">
										<i class="fa fa-calendar"></i>
									</div>
									<input type="text" class="form-control text-right datetimepicker" id="fdt_cbpaymentoth_datetime" name="fdt_cbpaymentoth_datetime" value=""/>
								</div>
                        ...
                        <div class="form-group">
							<label for="fst_cbpaymentoth_no" class="col-md-2 control-label"><?=lang("No. Pengeluaran")?></label>	
							<div class="col-md-10">				
								<input type="TEXT" id="fst_cbpaymentoth_no" name="fst_cbpaymentoth_no" class="form-control"  value="" placeholder="PREFIX/BRANCH/YEAR/MONTH/99999" /> 
							</div>
						</div>
						...
						<div class="form-group">
							<label for="fst_give_to" class="col-md-2 control-label"><?=lang("Dikeluarkan kepada")?> </label>
							<div class="col-sm-10">
                                <input type="text" class="form-control" id="fst_give_to"  name="fst_give_to"/>
                                <div id="fst_give_to_err" class="text-danger"></div>
                            </div> 
						</div>
						...
						<div class="form-group" style="margin-bottom:5px">
							...
							<div class="col-md-12">
								...
								<table id="tblcbpaymentotheritems" class="table table-bordered table-hover table-striped nowarp row-border" style="min-width:100%"></table>
							</div>		
	...
	<script type="text/javascript">
        var mdlAddItems = {
			...
			save:function(){
                ...
                var t =   $("#tblcbpaymentotheritems").DataTable();
...
<script type="text/javascript" info="event">
	$(function(){
		$("#btnNew").click(function(e){
			e.preventDefault();
			window.location.href = "<?=site_url()?>tr/kas_bank/pengeluaran_lain/add";
		});
		...
		$("#btnPrint").click(function(e){
			e.preventDefault();
			frameVoucher.print("<?=site_url()?>tr/kas_bank/pengeluaran_lain/print_voucher/" + $("#fin_cbpaymentoth_id").val());
		});
		$("#btnJurnal").click(function(e){
			e.preventDefault();
			MdlJurnal.showJurnalByRef("CBPO",$("#fin_cbpaymentoth_id").val());
		});
		...
		$("#btnClose").click(function(e){
			e.preventDefault();
			window.location.href = "<?=site_url()?>tr/kas_bank/pengeluaran_lain/";
		});

		$(".type-pembayaran").change(function(e){
			e.preventDefault();
		...
		$("#fin_kasbank_id").change(function(e){
			e.preventDefault();
			getTransactionNumber($("#fin_kasbank_id").val(),$("#fdt_cbpaymentoth_datetime").val());
			t = $("#tblcbreceiveothersitems").DataTable();
			t.clear().draw(false);
		});
...
<script type="text/javascript" info="init">
	$(function(){
		$("#fdt_cbpaymentoth_datetime").val(dateTimeFormat("<?= date("Y-m-d H:i:s")?>")).datetimepicker("update");	
		...
		$("#fin_kasbank_id").select2({
			ajax: {
				...
				data:function(params){
					params.kasbank_type = $(".type-pembayaran:checked").val();
					return params;
				},
		...
		$('#tblcbpaymentotheritems').on('preXhr.dt', function ( e, settings, data ) {
			...
			e.preventDefault();
			t = $("#tblcbpaymentotheritems").DataTable();
...
<script type="text/javascript" info="function">
	...
	function getTransactionNumber(kasBankId,cbReceiveDatetime){
		...
		App.getValueAjax({
			...
			callback:function(value){
				$("#fst_cbpaymentoth_no").val(value);
			}
		});
	}	

	function initForm(){
		if ($("#fin_cbpaymentoth_id").val() != 0){ //EDIT MODE
			//Get Data
			App.blockUIOnAjaxRequest("Get data transaction.....");
			$.ajax({
				url:"<?=site_url()?>tr/kas_bank/pengeluaran_lain/fetch_data/" + $("#fin_cbpaymentoth_id").val(),
				...
				if(resp.status =="SUCCESS"){
					...
					$("#fdt_cbpaymentoth_datetime").val(dateTimeFormat(dataH.fdt_cbpaymentoth_datetime)).datetimepicker("update");	
			...
			$(".type-pembayaran").trigger("change");
		}
	}

	function submitAjax(confirmEdit){
		if ($("#fin_kasbank_id").val() == null){
			alert("<?= lang('Pilih jenis pembayaran ...!') ?>");
			return;
		}
		...
		var details = $('#tblcbpaymentotheritems').DataTable();
		...
		mode = $("#fin_cbpaymentoth_id").val() == "0" ? "ADD" : "EDIT";	


		if (mode == "ADD"){
			url =  "<?= site_url() ?>tr/kas_bank/pengeluaran_lain/ajx_add_save/";
		}else{
			dataSubmit.push({
			...
			url =  "<?= site_url() ?>tr/kas_bank/pengeluaran_lain/ajx_edit_save/";
		}
	...
	function deleteAjax(confirmDelete){
		...
		var url =  "<?= site_url() ?>tr/kas_bank/pengeluaran_lain/delete/" + $("#fin_cbpaymentoth_id").val();
		$.ajax({
			url:url,
			method:"POST",
			data:dataSubmit,
