...
<section class="content-header">
	<h1><?=lang("Adjustment Stock")?><small><?=lang("form")?></small></h1>
	<ol class="breadcrumb">
		<li><a href="#"><i class="fa fa-dashboard"></i> <?= lang("Home") ?></a></li>
		<li><a href="#"><?= lang("Gudang") ?></a></li>
...
<section class="content">
    <div class="row">
        <div class="col-md-12">
            ...
            <!-- form start -->
            <form id="frmAdjHeader" class="form-horizontal" action="" method="POST" >
				<div class="box-body">
					<input type="hidden" id="frm-mode" value="<?=$mode?>">
					<input type="hidden" class="form-control" id="fin_adjustment_id" placeholder="<?=lang("(Autonumber)")?>" name="fin_adjustment_id" value="<?=$fin_adjustment_id?>" readonly>
					...
					<div class="form-group">
						...
						<label for="fdt_adjustment_datetime" class="col-md-2 control-label"><?=lang("Adjustment Date")?> *</label>
						<div class="col-md-4">
							<div class="input-group date">
								<div class="input-group-addon">
									<i class="fa fa-calendar"></i>
								</div>
								<input type="text" class="form-control text-right datetimepicker" id="fdt_adjustment_datetime" name="fdt_adjustment_datetime"/>
							</div>
					...
					<div class="form-group">
						...
						<label for="fst_reff" class="col-md-2 control-label"><?=lang("No. Ref")?></label>
						<div class="col-md-4">
							<input id="fst_reff" class="form-control" name="fst_reff" />
							<div id="fst_reff_err" class="text-danger"></div>
						</div>					
					</div>
                    ...
                    <div class="form-group">
						<div class="col-sm-6">	
							<div class="form-group">								
								<div class="col-sm-12">
									<label for="fst_notes" class=""><?=lang("Memo")?></label>
									<textarea class="form-control" id="fst_notes" placeholder="<?= lang("Memo") ?>" name="fst_notes" rows="5" style="resize:none"></textarea>
                        ...
                        <div style="border:1px inset #f0f0f0;border-radius:10px;padding:5px">
                            <fieldset style="padding:10px">
								<form class="form-horizontal">
                                    ...
                                    <div class="form-group">
										<label class="col-md-3 control-label"><?=lang("In/Out")?></label>	
										<div class="col-md-9">	
											<select id="fst_in_out" class="form-control" style="width:100%">
		...
		var mdlDetail = {
			show:function(){
				...
				if (selectedDetail != null){
                    ...
                    $("#fst_in_out").val(data.fst_in_out);
	...
	<script type="text/javascript" info="init">
		$(function(){
			$("#fstItem").select2({
				...
				ajax:{
					delay: 250,
					url: "<?=site_url()?>/tr/gudang/adjustment/ajxListItem",
					dataType: 'json',
			...
			$("#fstUnit").select2({
				...
				ajax:{
					url: "<?=site_url()?>/tr/gudang/adjustment/ajxListUnit",
	...
	<script type="text/javascript" info="function">
		...
		function showHideBatchSerial(){
			if (selectedItem.fbl_is_batch_number == 1){
				$(".batchNoBlock").show();
				getBatchNoList($("#fin_warehouse_id").val(),selectedItem.id, function(){});
			}else{
				$(".batchNoBlock").hide();
			...
			if (selectedItem.fbl_is_serial_number == 1){				
				$(".serialNoBlock").show();
				getSerialNoList($("#fin_warehouse_id").val(),selectedItem.id,$("#fstBatchNo").val(), function(){});
			}else{				
				$(".serialNoBlock").hide();
			}
		}
...
<script type="text/javascript" info="event">
	$(function(){
		$("#btnNew").click(function(e){
			e.preventDefault();
			window.location.replace("<?=site_url()?>tr/gudang/adjustment/add")
		});
		$("#btnPrint").click(function(e){
			e.preventDefault();
			frameVoucher.print("<?=site_url()?>tr/gudang/adjustment/print_voucher/" + $("#fin_adjustment_id").val());
		});
		...
		$("#btnList").click(function(e){
			e.preventDefault();
			window.location.replace("<?=site_url()?>tr/gudang/adjustment");
		});	
...
<script type="text/javascript" info="init">
	$(function(){
		$("#fdt_adjustment_datetime").val(dateTimeFormat("<?= date("Y-m-d H:i:s")?>")).datetimepicker("update");				
		$('#tblDetails').on('preXhr.dt', function ( e, settings, data ) {
			...
			columns:[
                ...
                {"title" : "Stock","width": "30px",data:"fst_in_out",orderable:false,},
...
<script type="text/javascript" info="function">
	function submitAjax(confirmEdit){  
		...
		data = $("#frmAdjHeader").serializeArray();
		...
		if ($("#fin_adjustment_id").val() == 0){
			url = "<?=site_url()?>tr/gudang/adjustment/ajx_add_save";
		}else{
			if (confirmEdit == 0){
			...
			url = "<?=site_url()?>tr/gudang/adjustment/ajx_edit_save";
		}		
        ...
        $.ajax({
            ...
            if (resp.message != "")	{
				...
				if(resp.status == "VALIDATION_FORM_FAILED" ){
					...
					data = resp.data;
					$("#fin_adjustment_id").val(data.insert_id);

					//Clear all previous error
					$(".text-danger").html("");
					// Change to Edit mode
					$("#frm-mode").val("EDIT");  //ADD|EDIT
					$('#fst_adjustment_no').prop('readonly', true);
				}
        });
	...
	function initForm(){
		if ($("#fin_adjustment_id").val() != 0){
			...
			$.ajax({
				url:"<?= site_url() ?>tr/gudang/adjustment/fetch_data/" + $("#fin_adjustment_id").val(),
			}).done(function(resp){
				data =  resp.data;
				dataH = data.header;
				if (dataH == null){
				...
				$("#fdt_adjustment_datetime").val(dateTimeFormat(dataH.fdt_adjustment_datetime)).datetimepicker("update");
	...
	function deleteAjax(confirmDelete){
		...
		var url =  "<?= site_url() ?>tr/gudang/adjustment/delete/" + $("#fin_adjustment_id").val();
		$.ajax({
			url:url,
			method:"POST",
			data:dataSubmit,
