...
<section class="content-header">
	<h1><?=lang("PAG Produksi")?><small><?=lang("form")?></small></h1>
	<ol class="breadcrumb">
		<li><a href="#"><i class="fa fa-dashboard"></i> <?= lang("Home") ?></a></li>
		<li><a href="#"><?= lang("Mutasi") ?></a></li>
            ...
            <form id="frmHeader" class="form-horizontal" action="" method="POST" >
				<div class="box-body">
                    ...
                    <div class="form-group">
						...
						<div class="col-md-4">
							<select  class="form-control" id="fin_mag_id" placeholder="<?=lang("MAG Produksi No")?>" name="fin_mag_id"></select>
                    ...
                    <div class="form-group">
						<label for="fst_wo_no" class="col-md-2 control-label"><?=lang("Work Order No.")?></label>
						<div class="col-md-10">							
							<label class="form-control" id="fst_wo_no" disabled></label>							
						</div>					
					</div>
...
<script type="text/javascript" info="init">
	$(function(){
		...
		$("#fin_mag_id").select2({
            ...
            ajax:{
                delay: 250,
                url: "<?=site_url()?>/tr/gudang/pagp/ajxListMAG",
                dataType: 'json',
		...
		$('#tblDetails').on('preXhr.dt', function ( e, settings, data ) {
			...
			columns:[
				...
				{"title" : "Qty Confirm","width": "50px",data:"fdb_qty_confirm",className:'text-right',orderable:false,visible:false,
					render:function(data,type,row){										
				...
				{"title" : "Action","width": "28px",className:'dt-body-center text-center',orderable:false,visible:false,
...
<script type="text/javascript" info="event">
	$(function(){
		$("#btnNew").click(function(e){
			e.preventDefault();
			window.location.replace("<?=site_url()?>tr/gudang/pagp/add")
		});
		...
		$("#btnList").click(function(e){
			e.preventDefault();
			window.location.replace("<?=site_url()?>tr/gudang/pagp");
		});	
...
<script type="text/javascript" info="function">
	...
	function loadMAGDetail(data){
        ...
        $("#fst_wo_no").text(header.fst_wo_no);
	...
	function getMAGDetail(callback){		
		$.ajax({
			url: "<?=site_url()?>/tr/gudang/pagp/ajxMAGDetail/" + selectedMAG.id,
			method:"GET",
		}).done(function(resp){
	...
	function submitAjax(confirmEdit){     
	   ...
	   */
		...
		if (mode == "ADD"){
			url = "<?=site_url()?>tr/gudang/pagp/ajx_add_save";
		}else{
			if (confirmEdit == 0){
			...
			url = "<?=site_url()?>tr/gudang/pagp/ajx_edit_save";
		}		
	...
	function initForm(){
		...
		if (mode == "EDIT"){			
			...
			$.ajax({
				url:"<?= site_url() ?>tr/gudang/pagp/fetch_data/" + finMagConfirmId,
			}).done(function(resp){
                ...
                $("#fst_wo_no").text(dataH.fst_wo_no);
