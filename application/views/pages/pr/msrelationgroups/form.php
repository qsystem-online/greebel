...
<section class="content">
    <div class="row">
        <div class="col-md-12">
            ...
            <!-- form start -->
            <form id="frmMSRelationgroups" class="form-horizontal" action="<?=site_url()?>pr/relation_group/add" method="POST" enctype="multipart/form-data">			
				<div class="box-body">
...
<script type="text/javascript">
	$(function(){
		<?php if($mode == "EDIT"){?>
			init_form($("#fin_relation_group_id").val());
		<?php } ?>
		...
		$("#btnSubmitAjax").click(function(event){
			...
			mode = $("#frm-mode").val();
			if (mode == "ADD"){
				url =  "<?= site_url() ?>pr/relation_group/ajx_add_save";
			}else{
			...
			$.ajax({
				...
				success: function (resp) {	
					...
					if(resp.status == "VALIDATION_FORM_FAILED" ){
						...
						data = resp.data;
						$("#fin_relation_group_id").val(data.insert_id);
						...
						$('#fst_relation_group_name').prop('readonly', true);
					}
				},
				error: function (e) {
		...
		$("#btnNew").click(function(e){
			e.preventDefault();
			window.location.replace("<?=site_url()?>pr/relation_group/add")
		});
		...
		$("#btnDelete").click(function(e){
			...
			$.ajax({
				url:"<?= site_url() ?>pr/relation_group/delete/" + $("#fin_relation_group_id").val(),
			}).done(function(resp){
				//consoleLog(resp);
				$.unblockUI();
				if (resp.message != "")	{
		...
		$("#btnList").click(function(e){
			e.preventDefault();
			window.location.replace("<?=site_url()?>pr/relation_group/lizt");
		});
	});

	function init_form(fin_relation_group_id){
		//alert("Init Form");
		var url = "<?=site_url()?>pr/relation_group/fetch_data/" + fin_relation_group_id;
		$.ajax({
			type: "GET",
			url: url,
			success: function (resp) {	
				console.log(resp.ms_relationgroups);

				$.each(resp.ms_relationgroups, function(name, val){
					var $el = $('[name="'+name+'"]'),
					type = $el.attr('type');
					switch(type){
						case 'checkbox':
