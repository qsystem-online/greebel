		...
		<div class="col-md-12">
			...
			<form id="frmUser" class="form-horizontal" action="<?=site_url()?>user/add" method="POST" enctype="multipart/form-data">				
				<div class="box-body">
					...
					<div class="form-group sales-area" style="display:none" >
						
						<label  class="col-md-2 control-label"><?=lang("Sales Area")?></label>												
						<div class="col-md-3">
							<select id="fst_national_area" class="form-control" >
								<?php
									$nationalList = $this->mssalesnational_model->getList();
									foreach($nationalList as $national){
										echo "<option value='$national->fin_sales_national_id'>$national->fst_name</option>";
									}
								?>
							</select>
						</div>
						<div class="col-md-3">
							<select id="fst_regional_area" class="form-control"></select>
						</div>
						<div class="col-md-3">
							<select id="fin_sales_area_id" name="fin_sales_area_id" class="form-control"></select>
						</div>											
					</div>
...
<script type="text/javascript">
	var salesDeptId = "<?= $finSalesDepartmentId ?>";
	...
	$(function() {
		...
		$("#select-departmentname").select2({
			...
			if ($("#select-departmentname").val() == salesDeptId ){
				$(".sales-area").show();
			}else{
				$(".sales-area").hide();
				$("#fin_sales_area_id").val(null).trigger("change");
			}
		});

		$("#fst_national_area").select2({
			width: '100%',
			allowClear: true,           
			placeholder: 'National',
		}).on("change",function(e){
			$("#fst_regional_area").val(null).trigger("change");
			$("#fin_sales_area_id").val(null).trigger("change");
		});

		$("#fst_regional_area").select2({
			width: '100%',			
			allowClear: true,           
			placeholder: 'Regional',
			ajax:{
				url: '<?= site_url() ?>master/sales_area/ajx_get_regional',
    			dataType: 'json',
				data:function(params){
					params.fin_sales_national_id = $("#fst_national_area").val();
					return params;
				},
		...
		$("#fin_sales_area_id").select2({
			width: '100%',			
			allowClear: true,           
			placeholder: 'Area',
			ajax:{
				url: '<?= site_url() ?>master/sales_area/ajx_get_area',
    			dataType: 'json',
				data:function(params){
					params.fin_sales_regional_id = $("#fst_regional_area").val();
					return params;
				},
				processResults: function (resp) {
					var data = resp.data;
	...
	function init_form(fin_user_id){
		...
		$.ajax({
			...
			success: function (resp) {	
				...
				if (resp.user.fin_sales_national_id == null){
					$("#fst_national_area").val(null).trigger("change");
				}else{
					App.addOptionIfNotExist("<option value='"+ resp.user.fin_sales_national_id  +"'>"+resp.user.fst_sales_national_name+"</option>","fst_national_area");
				}

				if (salesDeptId == $("#select-departmentname").val()){
					$(".sales-area").show();
				}else{
					$(".sales-area").hide();
				}

				if (resp.user.fin_sales_regional_id == null){
					$("#fst_regional_area").val(null).trigger("change");
				}else{
					App.addOptionIfNotExist("<option value='"+ resp.user.fin_sales_regional_id  +"'>"+resp.user.fst_sales_regional_name+"</option>","fst_regional_area");
				}

				if (resp.user.fin_sales_area_id == null){
					$("#fin_sales_area_id").val(null).trigger("change");
				}else{
					App.addOptionIfNotExist("<option value='"+ resp.user.fin_sales_area_id  +"'>"+resp.user.fst_sales_area_name+"</option>","fin_sales_area_id");
				}
