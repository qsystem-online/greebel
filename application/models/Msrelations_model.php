...
clASs Msrelations_model extends MY_Model {
    ...
    public function getDataById($fin_relation_id){
        ...
        MID(a.fst_area_code, 1, 13) AS village,b.fst_nama AS fst_province_name,c.fst_nama AS fst_district_name,d.fst_nama AS fst_subdistrict_name,
        e.fst_nama AS fst_village_name FROM msshippingaddress a
        ...
        WHERE a.fin_relation_id = ? AND fst_active = 'A' ";*/
        $qr = $this->db->query($ssql, [$fin_relation_id]);
        $rsShipping = $qr->result();

        /*$arrTmp = explode(".",$rsShipping->district);
        if (sizeof($arrTmp) == 2 ){
            $arrTmp = explode(".",$rsShipping->subdistrict);
            if (sizeof($arrTmp) == 3){
                $arrTmp = explode(".",$rsShipping->village);
                if (sizeof($arrTmp) != 4){
                    $rsShipping->village = null;
                    $rsShipping->fst_village_name = null;
                }
            }else{
                $rsShipping->subdistrict = null;
                $rsShipping->fst_subdistrict_name = null;
                $rsShipping->village = null;
                $rsShipping->fst_village_name = null;
            }
        }else{
            $rsShipping->district = null;
            $rsShipping->fst_district_name = null;
            $rsShipping->subdistrict = null;
            $rsShipping->fst_subdistrict_name = null;
            $rsShipping->village = null;
            $rsShipping->fst_village_name = null;
