...
class Invoice_outstanding_rpt_model extends CI_Model {
    ...
    public function queryComplete($data, $sorder_by="b.fst_inv_no", $rptLayout="1") {
        ...
        if ($rptLayout == "1" || $rptLayout == "2" || $rptLayout == "3" ){
            ...
            if ($area_code > "0") {
                $swhere .= " AND c.fst_area_code LIKE '" . $area_code ."%'";
            }
        }
        if ($rptLayout == "5"){
            if ($branch_id > "0") {
        ...
        if ($sorder_by != "") {
            if ($rptLayout == "3")
                ...
                switch (strlen($area_code)){
                    case "2":
                        $sorderby = " ORDER BY e.fst_nama,b.fin_relation_id";
                        break;
                    case "5":
                        $sorderby = " ORDER BY f.fst_nama,b.fin_relation_id";
                        break;
                    case "8":
                        $sorderby = " ORDER BY g.fst_nama,b.fin_relation_id";
                        break;
                    case "13":
                        $sorderby = " ORDER BY " .$sorder_by;
                        break;
        ...
        switch($rptLayout) {
            case "1":
                $ssql = "SELECT b.fin_sales_id as Sales_Id,a.fst_username as Sales_Name,b.fin_relation_id as Relation_Id,c.fst_relation_name as Relation_Name,b.fin_inv_id as Id_Inv,b.fst_inv_no as No_Inv, b.fdt_inv_datetime as Inv_Date, b.fin_terms_payment as TOP,
                ...
                (b.fdc_total - b.fdc_total_return) as Total_Netto,b.fin_warehouse_id,
                IFNULL(MID(c.fst_area_code, 1, 2),'') AS provinces,IFNULL(MID(c.fst_area_code, 1, 5),'') AS district,IFNULL(MID(c.fst_area_code, 1, 8),'') AS subdistrict,IFNULL(MID(c.fst_area_code, 1, 13),'') AS village,
                d.fst_nama AS fst_province_name,e.fst_nama AS fst_district_name,f.fst_nama AS fst_subdistrict_name,g.fst_nama AS fst_village_name
                FROM users a RIGHT OUTER JOIN 
                (SELECT * FROM trinvoice WHERE fst_active !='D') b ON a.fin_user_id = b.fin_sales_id LEFT OUTER JOIN
                msrelations c ON b.fin_relation_id = c.fin_relation_id
                LEFT OUTER JOIN msarea d ON MID(c.fst_area_code, 1, 2) = d.fst_kode
                LEFT OUTER JOIN msarea e ON MID(c.fst_area_code, 1, 5) = e.fst_kode
                LEFT OUTER JOIN msarea f ON MID(c.fst_area_code, 1, 8) = f.fst_kode
                LEFT OUTER JOIN msarea g ON MID(c.fst_area_code, 1, 13) = g.fst_kode "
                . $swhere . $sorderby;
                break;
            case "2":
                $ssql = "SELECT b.fin_sales_id as Sales_Id,a.fst_username as Sales_Name,b.fin_relation_id as Relation_Id,c.fst_relation_name as Relation_Name,b.fin_inv_id as Id_Inv,b.fst_inv_no as No_Inv, b.fdt_inv_datetime as Inv_Date, b.fin_terms_payment as TOP,
                ...
                IFNULL(MID(c.fst_area_code, 1, 2),'') AS provinces,IFNULL(MID(c.fst_area_code, 1, 5),'') AS district,IFNULL(MID(c.fst_area_code, 1, 8),'') AS subdistrict,IFNULL(MID(c.fst_area_code, 1, 13),'') AS village,
                d.fst_nama AS fst_province_name,e.fst_nama AS fst_district_name,f.fst_nama AS fst_subdistrict_name,g.fst_nama AS fst_village_name,
                h.fst_cbreceive_no as Receive_No,h.fdt_cbreceive_datetime as Receive_Date,h.fdc_receive_amount as Receive_Amount
                FROM users a RIGHT OUTER JOIN 
                (SELECT * FROM trinvoice WHERE fst_active !='D') b ON a.fin_user_id = b.fin_sales_id LEFT OUTER JOIN
                msrelations c ON b.fin_relation_id = c.fin_relation_id
                LEFT OUTER JOIN msarea d ON MID(c.fst_area_code, 1, 2) = d.fst_kode
                ...
                LEFT OUTER JOIN msarea f ON MID(c.fst_area_code, 1, 8) = f.fst_kode
                LEFT OUTER JOIN msarea g ON MID(c.fst_area_code, 1, 13) = g.fst_kode
                LEFT OUTER JOIN (SELECT a.fst_cbreceive_no,a.fdt_cbreceive_datetime,b.fst_trans_type,b.fin_trans_id,b.fdc_receive_amount FROM trcbreceive a 
                LEFT OUTER JOIN trcbreceiveitems b ON a.fin_cbreceive_id = b.fin_cbreceive_id WHERE a.fst_active !='D' AND b.fst_trans_type ='INV_SO') h ON b.fin_inv_id = h.fin_trans_id "
                . $swhere . $sorderby;
                break;
            case "3":
                $ssql = "SELECT b.fin_sales_id as Sales_Id,a.fst_username as Sales_Name,b.fin_relation_id as Relation_Id,c.fst_relation_name as Relation_Name,b.fin_inv_id as Id_Inv,b.fst_inv_no as No_Inv, b.fdt_inv_datetime as Inv_Date, b.fin_terms_payment as TOP,
