...
class Kasbank_rpt_model extends CI_Model {
    ...
    public function queryComplete($data, $sorder_by="a.fst_glaccount_code", $rptLayout="1") {
        ...
        if ($swhere != "") {
            $swhere = " WHERE (a.fst_glaccount_level ='DB' OR  a.fst_glaccount_level ='DK') AND " . substr($swhere, 5);
        }
        if ($sorder_by != "") {
        ...
        switch($rptLayout) {
            case "1":
                ...
                FROM (SELECT * FROM glaccounts WHERE (fst_glaccount_level = 'DK' OR fst_glaccount_level = 'DB') AND fst_active !='D') a 
