...
class Piutang_rpt_model extends CI_Model {
    ...
    public function queryComplete($data, $sorder_by="a.fst_glaccount_code", $rptLayout="1") {
        ...
        if (isset($data['kasbank_code'])) { $kasbank_code = $data['kasbank_code'];}
        ...
        switch($rptLayout) {
            ...
            //case "3":
            //    $ssql = "SELECT a.fin_sales_id,d.fst_username AS Sales_Name,a.fin_relation_id,a.fst_relation_name,c.fst_orgi_curr_code,
            //    c.fin_branch_id,IFNULL(c.fdc_debit,0) AS Total_Debit,IFNULL(c.fdc_credit,0) AS Total_Credit,IFNULL(b.SA_Debit,0) AS SA_Debit,IFNULL(b.SA_Credit,0) AS SA_Credit
            //    FROM (SELECT * FROM msrelations WHERE fst_active !='D') a 
            //    LEFT OUTER JOIN (SELECT fin_relation_id,SUM(fdc_debit) AS SA_Debit, SUM(fdc_credit) AS SA_Credit  FROM glledger WHERE fst_active !='D' AND fdt_trx_datetime < '" . date('Y-m-d', strtotime($start_date)) . "' GROUP BY fin_relation_id) b
            //    ON a.fin_relation_id = b.fin_relation_id
            //    LEFT OUTER JOIN (SELECT * FROM glledger WHERE fst_active !='D' AND fdt_trx_datetime >= '" . date('Y-m-d', strtotime($start_date)) . "' AND fdt_trx_datetime <= '". date('Y-m-d 23:59:59', strtotime($end_date)). "' ) c 
            //    ON a.fin_relation_id = c.fin_relation_id INNER JOIN users d
            //    ON a.fin_sales_id = d.fin_user_id $swhere GROUP BY a.fin_relation_id ORDER BY a.fin_sales_id,a.fin_relation_id";
            //    break;
            default:
                break;
        }
