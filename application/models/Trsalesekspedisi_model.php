...
class Trsalesekspedisi_model extends MY_Model {
	...
	public function getDataVoucher($finSalesEkspedisiId){
		$ssql = "SELECT a.*,
			b.fst_relation_name AS fst_customer_name,
			c.fst_relation_name AS fst_supplier_name,
			...
			INNER JOIN msshippingaddress d ON a.fin_shipping_address_id = d.fin_shipping_address_id 
			WHERE fin_salesekspedisi_id = ?";
		$qr = $this->db->query($ssql,[$finSalesEkspedisiId]);
		$header  = $qr->row_array();

		$ssql = "SELECT a.*,b.fst_sj_no
			FROM trsalesekspedisiitems a 
			INNER JOIN trsuratjalan b on a.fin_sj_id = b.fin_sj_id
			where a.fin_salesekspedisi_id = ?";
		$qr = $this->db->query($ssql,[$finSalesEkspedisiId]);
