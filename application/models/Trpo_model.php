...
class Trpo_model extends MY_Model {
	...
	public function posting($finPOId){
		...
		$finProcessId = $dataH["fin_pr_process_id"]; //$this->input->post("fin_process_id");
		if ($finProcessId != 0 && $finProcessId != null ){
			$ssql = "UPDATE trpurchaserequestprocess set fin_po_id = ? where fin_process_id = ?";
	...
	public function getHeaderProcessPR($finProcessId){
		$ssql ="SELECT * FROM trpurchaserequestprocess where fin_process_id = ?";
	...
	public function getDetailPr($finProcessId){
		...
		*/
		...
		for($i = 0;$i < sizeof($rs);$i++){			
			$rs[$i]->fdc_last_buy_price = $this->getLastBuyPrice($rs[$i]->fin_item_id,$rs[$i]->fst_unit);
		}
