...
class Sales_order_model extends MY_Model {
    ...
    public function getDataPromo($fin_customer_id,$details,$trxDate=null){
        ...
        $ssql = "select a.*,b.MemberGroupId from msrelations a 
            left join (select * from msmemberships where fst_active = 'A' and ExpiryDate > ?) b on a.RelationId = b.RelationId 
            where a.RelationId = ?";
        ...
        $relationGroupId = $rwCek->RelationGroupId;
        $memberGroupId = $rwCek->MemberGroupId;
        ...
        for($i = 0 ; $i < sizeof($rs); $i++){              
            ...
            $ssql = "select count(*) as ttlParticipant from mspromoitemscustomer 
                ...
                and fin_customer_id = ?";                
            $qr = $this->db->query($ssql,[$promoId,$relationGroupId]);            
            $rwParticipant = $qr->row();
            if($rwParticipant->ttlParticipant > 0){
                $rs[$i]->isParticipant = true;
            }            
        ...
        if ($memberGroupId != null){
            for($i = 0 ; $i < sizeof($rs); $i++){              
                ...
                $ssql = "select count(*) as ttlParticipant from mspromoitemscustomer 
                    ...
                    and fin_customer_id = ?";                
                $qr = $this->db->query($ssql,[$promoId,$memberGroupId]);
                $rwParticipant = $qr->row();
                if($rwParticipant->ttlParticipant > 0){
                    $rs[$i]->isParticipant = true;
                }            
        ...
        for($i = 0 ; $i < $ttlRow; $i++){             
            ...
            if($minTotalPurchase > 0){ //Proses Berdasarkan Total Belanja                
                ...
                for($y=0;$y<sizeof($details);$y++){
                    ...
                    $rwItem = $qr->row();
                    if(!$rwItem){
                        $details[$y]->fin_itemsubgroup_id = null; 
                    }else{
                        $details[$y]->fin_itemsubgroup_id = $rwItem->ItemSubGroupId;                    
                    }
                }
                ...
                if ($rwPromo->fbl_qty_gabungan){ //Proses Qty Gabungan                    
                    ...
                    foreach($details as $item){
                        ...
                        if(sizeof($itemRules) == 0){
                            ...
                            $isOnRules = $this->isOnPromoItemRules($itemRules,$item->fin_item_id,$item->fin_itemsubgroup_id);
                        }
                        if($isOnRules){    
                    ...
                    foreach($itemRules as $rule){
                        if($rule->fst_item_type == "SUB GROUP"){
                            $totalQtySubGroup = 0; 
                            foreach($details as $item){
                                if ($item->fin_itemsubgroup_id == $rule->fin_item_id && $item->fst_unit == $rule->fst_unit){
        ...
        $ssql = "select *,b.ItemName as fst_item_name,b.ItemCode as fst_item_code from mspromo a 
            left join msitems b on a.fin_promo_item_id = b.ItemId 
    ...
    private function isOnPromoItemRules($rules,$fin_item_id,$fin_itemsubgroup_id){
        foreach($rules as $rule){
            if($rule->fst_item_type == 'SUB GROUP'){
                if ($fin_itemsubgroup_id == $fin_item_id){
                    return true;
                }
            }else if ($rule->fst_item_type == 'ITEM') {
