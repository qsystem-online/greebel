...
class Penerimaan_mutasi extends MY_Controller{
	...
	public function index(){
		...
		$this->list['boxTools'] = [
			"<a id='btnNew'  href='".site_url()."tr/gudang/penerimaan_mutasi/add' class='btn btn-primary btn-sm'><i class='fa fa-plus' aria-hidden='true'></i> New Record</a>",
		...
		$this->list['pKey'] = "id";
		$this->list['fetch_list_data_ajax_url'] = site_url() . 'tr/gudang/penerimaan_mutasi/fetch_list_data';
		$this->list['arrSearch'] = [
		...
		$this->list['columns'] = [
			...
			['title' => 'No. PAG', 'width' => '60px', 'data' => 'fst_mag_confirm_no'],
			['title' => 'Tanggal', 'width' => '50px', 'data' => 'fdt_mag_confirm_datetime'],
			...
			['title' => 'Action', 'width' => '50px', 'sortable' => false, 'className' => 'text-center',
				'render'=>"function(data,type,row){
					...
					action += '<a class=\"btn-edit\" href=\"".site_url()."tr/gudang/penerimaan_mutasi/edit/' + row.fin_mag_confirm_id + '\" data-id=\"\"><i class=\"fa fa-pencil\"></i></a>&nbsp;';
	...
	private function openForm($mode = "ADD", $finPagId = 0){
		...
		$data["mode"] = $mode;
		$data["title"] = $mode == "ADD" ? lang("Penerimaan Mutasi Antar Gudang") : lang("Update Penerimaan Mutasi Antar Gudang");
		...
		$page_content = $this->parser->parse('pages/tr/gudang/mutasi/form_confirm', $data, true);
		$main_footer = $this->parser->parse('inc/main_footer', [], true);
	...
	public function ajx_edit_save(){	
		...
		try{			
			...
			$resp = $this->trmagconfirm_model->isEditable($finMagConfirmId);
			if ($resp["status"] != "SUCCESS"){
				throw new CustomException($resp["message"],3003,"FAILED",null);
			}			
		}catch(CustomException $e){
	...
	public function ajxListMAG(){
		...
		$ssql ="select a.fin_mag_id,a.fst_mag_no
			from trmag a 
			where fbl_closed = false and fst_mag_no like ?";
		$qr = $this->db->query($ssql,['%'.$term.'%']);
		$this->json_output([
			"status"=>"SUCCESS",
			"data"=>$qr->result()
