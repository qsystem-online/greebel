...
class Assembling extends MY_Controller{
	...
	public function index(){
		...
		$this->list['boxTools'] = [
			"<a id='btnNew'  href='".site_url()."tr/production/assembling/add' class='btn btn-primary btn-sm'><i class='fa fa-plus' aria-hidden='true'></i> New Record</a>",
	...
	private function openForm($mode = "ADD", $finId = 0){
		...
		$data["mode"] = $mode;
		$data["title"] = $mode == "ADD" ? lang("Add Assembling /Disassembling") : lang("Update Assembling /Disassembling");
		...
		$page_content = $this->parser->parse('pages/tr/production/assembling/form', $data, true);
		$main_footer = $this->parser->parse('inc/main_footer', [], true);
