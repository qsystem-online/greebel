...
class Rekap_biaya extends MY_Controller
{

	public $menuName="report_rekap_biaya";
	...
	public function __construct()
	{
		...
		$this->load->model('vmodels/rekapbiaya_rpt_model');
		$this->load->model('glaccounts_model');
		$this->load->model('profitcostcenter_model');

		$this->layout_columns = [
            ['layout' => 1, 'label'=>'Kode Akun', 'value'=>'0', 'selected'=>false,'sum_total'=>false],
			['layout' => 1, 'label'=>'Nama Akun', 'value'=>'1', 'selected'=>false,'sum_total'=>false],
			['layout' => 1, 'label'=>'Jumlah', 'value'=>'2', 'selected'=>false,'sum_total'=>false],					
	...
	public function loadForm()
	{
		...
		$side_filter = $this->parser->parse('reports/kas_bank/rekap_biaya/form',$this->data, true);
		$this->data['REPORT_FILTER'] = $side_filter;
		$this->data['TITLE'] = "KARTU KAS/BANK REPORT";
		$mode = "Report";
		// $this->data["mode"] = $mode;
	...
	public function process()
	{
		...
		$this->load->model('rekapbiaya_rpt_model');
		$this->form_validation->set_rules($this->rekapbiaya_rpt_model->getRules());
	...
	public function generateReport($isPreview = 1){		
		...
		$dataReport = $this->rekapbiaya_rpt_model->queryComplete($data,"b.fin_rec_id",$data['rpt_layout']);
		...
		switch ($data['rpt_layout']){
			case "1":
				$this->parser->parse('reports/kas_bank/rekap_biaya/layout1', ["selectedCols"=>$selectedCols,"ttlCol"=>$totalColumn,"dataReport"=>$dataReport]);
				break;
			default:
				$this->parser->parse('reports/kas_bank/rekap_biaya/layout1', ["selectedCols"=>$selectedCols,"ttlCol"=>$totalColumn,"dataReport"=>$dataReport]);
				break;
