...
class Kartu_kasbank extends MY_Controller
{

	public $menuName="report_kartu_kasbank";
	...
	public function __construct()
	{
		...
		$this->load->model('vmodels/kasbank_rpt_model');
	...
	public function loadForm()
	{
		...
		$side_filter = $this->parser->parse('reports/kas_bank/kartu_kasbank/form',$this->data, true);
	...
	public function process()
	{
		...
		$this->load->model('kasbank_rpt_model');
		$this->form_validation->set_rules($this->kasbank_rpt_model->getRules());
	...
	public function generateReport($isPreview = 1){		
		...
		$data = [
			...
			"fst_account_code" => $this->input->post("fst_account_code"),
            "fst_account_code2" => $this->input->post("fst_account_code2"),
		...
		$dataReport = $this->kasbank_rpt_model->queryComplete($data,"b.fin_rec_id",$data['rpt_layout']);
		...
		switch ($data['rpt_layout']){
			case "1":
				$this->parser->parse('reports/kas_bank/kartu_kasbank/layout1', ["selectedCols"=>$selectedCols,"ttlCol"=>$totalColumn,"dataReport"=>$dataReport]);
				break;
			default:
				$this->parser->parse('reports/kas_bank/kartu_kasbank/layout1', ["selectedCols"=>$selectedCols,"ttlCol"=>$totalColumn,"dataReport"=>$dataReport]);
				break;
