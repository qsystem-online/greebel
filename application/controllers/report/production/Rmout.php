...
class Rmout extends MY_Controller
{
	...
	public function loadForm()
	{
		...
		$side_filter = $this->parser->parse('reports/production/rmout/form',$this->data, true);
		$this->data['REPORT_FILTER'] = $side_filter;
		$this->data['TITLE'] = "LAPORAN RM-OUT";
		$mode = "Report";
		// $this->data["mode"] = $mode;
	...
	public function process()
	{
		...
		$this->load->model('trrmout_rpt_model');
		$this->form_validation->set_rules($this->trrmout_rpt_model->getRules());
	...
	public function generateReport($isPreview = 1){		
		...
		$dataReport = $this->trrmout_rpt_model->queryComplete($data,"a.fin_rmout_id",$data['rpt_layout']);
		...
		if ($data['rpt_layout'] == 1){
			$this->parser->parse('reports/production/rmout/layout1', ["selectedCols"=>$selectedCols,"ttlCol"=>$totalColumn,"dataReport"=>$dataReport]);
		}else if($data['rpt_layout'] == 2){
			$this->parser->parse('reports/production/rmout/layout2', ["selectedCols"=>$selectedCols,"ttlCol"=>$totalColumn,"dataReport"=>$dataReport]);
		}else if($data['rpt_layout'] == 3){
			$this->parser->parse('reports/production/rmout/layout3', ["selectedCols"=>$selectedCols,"ttlCol"=>$totalColumn,"dataReport"=>$dataReport]);
		}
