...
class Lpb_purchase extends MY_Controller{
	...
	public function __construct()
	{
		...
		$this->load->model('vmodels/trlpbpurchase_rpt_model');
	...
	public function loadForm()
	{
		...
		$side_filter = $this->parser->parse('reports/lpb_purchase/form',$this->data, true);
		$this->data['REPORT_FILTER'] = $side_filter;
		$this->data['TITLE'] = "LPB PURCHASE REPORT";
		$mode = "Report";
		// $this->data["mode"] = $mode;
	...
	public function process()
	{
		...
		$this->load->model('trlpbpurchase_rpt_model');
		$this->form_validation->set_rules($this->trlpbpurchase_rpt_model->getRules());
	...
	public function generateReport($isPreview = 1){		
		...
		$dataReport = $this->trlpbpurchase_rpt_model->queryComplete($data,"a.fin_lpbpurchase_id",$data['rpt_layout']);
		...
		switch ($data['rpt_layout']){
			case "1":
				$this->parser->parse('reports/lpb_purchase/layout1', ["selectedCols"=>$selectedCols,"ttlCol"=>$totalColumn,"dataReport"=>$dataReport]);
				break;
			case "2":
				$this->parser->parse('reports/lpb_purchase/layout2', ["selectedCols"=>$selectedCols,"ttlCol"=>$totalColumn,"dataReport"=>$dataReport]);
				break;
			case "3":
				$this->parser->parse('reports/lpb_purchase/layout3', ["selectedCols"=>$selectedCols,"ttlCol"=>$totalColumn,"dataReport"=>$dataReport]);
				break;
			case "4":
				$this->parser->parse('reports/lpb_purchase/layout4', ["selectedCols"=>$selectedCols,"ttlCol"=>$totalColumn,"dataReport"=>$dataReport]);
				break;
			default:
				$this->parser->parse('reports/lpb_purchase/layout1', ["selectedCols"=>$selectedCols,"ttlCol"=>$totalColumn,"dataReport"=>$dataReport]);
				break;
