...
class Sales_order extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->layout_columns = [
            ['layout' => 1, 'label'=>'Nou.', 'value'=>'A', 'selected'=>false,'sum_total'=>false],
            ['layout' => 1, 'label'=>'No.SO', 'value'=>'B', 'selected'=>false,'sum_total'=>false],
            ['layout' => 1, 'label'=>'Tgl.SO', 'value'=>'C', 'selected'=>false,'sum_total'=>false],
            ['layout' => 1, 'label'=>'Pelanggan', 'value'=>'D', 'selected'=>false,'sum_total'=>false],
            ['layout' => 1, 'label'=>'Sales', 'value'=>'E', 'selected'=>false,'sum_total'=>false],
            ['layout' => 1, 'label'=>'Kode Barang', 'value'=>'F', 'selected'=>false,'sum_total'=>false],
            ['layout' => 1, 'label'=>'Nama Barang', 'value'=>'G', 'selected'=>false,'sum_total'=>false],
            ['layout' => 1, 'label'=>'Qty', 'value'=>'H', 'selected'=>false,'sum_total'=>true],
            ['layout' => 1, 'label'=>'Unit', 'value'=>'I', 'selected'=>false,'sum_total'=>false],
            ['layout' => 1, 'label'=>'Harga', 'value'=>'J', 'selected'=>false,'sum_total'=>false],
            ['layout' => 1, 'label'=>'Diskon', 'value'=>'K', 'selected'=>false,'sum_total'=>false],
            ['layout' => 1, 'label'=>'Jumlah', 'value'=>'L', 'selected'=>false,'sum_total'=>true],
            ...
            if ($dataReport==[]) {
                ...
                if  ($data['rpt_layout'] ==  1){
                    ...
                    */
                    $y =0;
                    for($i=0;$i<12;$i++){
                        $col = $this->phpspreadsheet->getNameFromNumber($y);
                        if (in_array($col, $data['selected_columns'][0])) {
                            $y++;    
                    ...
                    $ly1 = [];
                    $sumCol =0;
                    foreach ($this->layout_columns  as $ly){
                        if ($ly["layout"] == 1){
                            if (in_array($ly["value"],$data['selected_columns'][0])){
                                $sumCol++;
                            }

                            if ($ly["sum_total"] == true){
                                $sumCol--;
                                break;
                            }
                        }
                    ...
                    //Merged Data                               
                    var_dump($ttlCol);
                    $sumCol =$this->phpspreadsheet->getNameFromNumber($sumCol-1);
                    $ttlCol =$this->phpspreadsheet->getNameFromNumber($ttlCol-1);
                    var_dump($ttlCol);
                    var_dump($sumCol);
                    die();
