...
class Buku_besar extends MY_Controller
{
	...
	public $menuName="report_buku_besar";
	...
	public function __construct()
	{
		...
		$this->load->model('vmodels/bukubesar_rpt_model');
	...
	public function loadForm()
	{
		...
		$side_filter = $this->parser->parse('reports/akuntansi/buku_besar/form',$this->data, true);
		$this->data['REPORT_FILTER'] = $side_filter;
		$this->data['TITLE'] = "BUKU BESAR G/L";
		$mode = "Report";
		// $this->data["mode"] = $mode;
	...
	public function process()
	{
		...
		$this->load->model('bukubesar_rpt_model');
		$this->form_validation->set_rules($this->bukubesar_rpt_model->getRules());
	...
	public function generateReport($isPreview = 1){		
		...
		$data = [
            ...
            "fst_keyword" => $this->input->post("fst_keyword"),
			"rpt_layout" => $this->input->post("rpt_layout"),
			"selected_columns" => array($this->input->post("selected_columns"))
		];		
		$dataReport = $this->bukubesar_rpt_model->queryComplete($data,"b.fin_rec_id",$data['rpt_layout']);
		...
		switch ($data['rpt_layout']){
			case "1":
				$this->parser->parse('reports/akuntansi/buku_besar/layout1', ["selectedCols"=>$selectedCols,"ttlCol"=>$totalColumn,"dataReport"=>$dataReport]);
				break;
			default:
				$this->parser->parse('reports/akuntansi/buku_besar/layout1', ["selectedCols"=>$selectedCols,"ttlCol"=>$totalColumn,"dataReport"=>$dataReport]);
				break;
